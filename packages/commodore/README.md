...
<p align="center" width="100%">
    <img height="90" src="https://raw.githubusercontent.com/hyperweb-io/dev-utils/refs/heads/main/docs/img/commodore.svg" />
</p>
...
<p align="center" width="100%">
  ...
  <a href="https://www.npmjs.com/package/commodore"><img height="20" src="https://img.shields.io/npm/dt/commodore"></a>
  <a href="https://www.npmjs.com/package/commodore"><img height="20" src="https://img.shields.io/github/package-json/v/hyperweb-io/dev-utils?filename=packages%2Fcommodore%2Fpackage.json"></a>
</p>
...
- [API Reference](#api-reference)
  - [Commodore Class](#commodore-class)
...
When running in CI/CD or without a TTY, commodore automatically falls back to default values:
...
const prompter = new Commodore(options);
```
...
When working with multiple `Commodore` instances that share the same input stream (typically `process.stdin`), only one instance should be actively prompting at a time. Each instance attaches its own keyboard listener, so having multiple active instances will cause duplicate or unexpected keypress behavior.

**Best practices:**

1. **Reuse a single instance** - Create one `Commodore` instance and reuse it for all prompts:
   ```typescript
   const prompter = new Commodore();
...
```typescript
import { Commodore, Question } from 'commodore';
...
```typescript
import { CLI, CommandHandler } from 'commodore';
import { Question } from 'commodore';
...
Commodore comes with several built-in resolvers ready to use:
...
When resolving default values, commodore follows this priority:
...
For isolated resolver registries, use a custom resolver registry per Commodore instance:

```typescript
import { DefaultResolverRegistry, Commodore } from 'commodore';
...
Resolvers fail silently by default. If a resolver throws an error or returns `undefined`, commodore falls back to the static `default` value (if provided):
...
For debugging, set `DEBUG=commodore` to see resolver errors:

```bash
DEBUG=commodore node your-cli.js
```

### Real-World Use Case

```typescript
import { Commodore, registerDefaultResolver } from 'commodore';
...
```typescript
import { CLI, CommandHandler, CLIOptions } from 'commodore';
